...
Pi Web Space treats an explicitly empty tool selection as **Chat only**, not merely as
...
project `AGENTS.md`, `AGENTS.override.md`, and `CLAUDE.md` files. Pi Web Space does not
...
tools are activated alongside the profile's built-in tools except for Pi Web Space's
