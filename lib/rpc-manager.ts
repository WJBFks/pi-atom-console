...
export class AgentSessionWrapper {
      ...
      if (typeof this.inner.bindExtensions === "function") {
        ...
        await bindExtensions.call(this.inner, {
          ...
          shutdownHandler: () => this.emit({
            ...
            method: "notify",
            notifyType: "warning",
            message: "Extension requested shutdown, but shutdown is not supported in Pi Web Space.",
  ...
  private createExtensionUiContext(): ExtensionUiContextLike {
    return {
      ...
      getAllThemes: () => [],
      getTheme: () => undefined,
      setTheme: () => ({ success: false, error: "Theme switching is not supported in Pi Web Space extension UI yet" }),
...
export async function startRpcSession(
  ...
  const starting = (async () => {
    ...
    if (!subagentResources && selectedToolNames !== undefined) {
      ...
      // tool registry — so they were unavailable in Pi Web Space sessions even though the
    ...
    // extensions stay usable in Pi Web Space just like in the `pi` CLI.
